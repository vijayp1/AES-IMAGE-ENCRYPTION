...
public class encryption {
	...
	public static String[][] encrypt(String inpt,String[][] state){
	...
	public static String checkHex(String a) {
		if(a.length()==1)
			a="0"+a;
		a=a.toUpperCase();
		return a;
	...
	public static void main(String args[])throws Exception {
      ...
      File file= new File("G:/assignment/sem 5/ISAA/PROJECT/New folder/s.jpg");
      ...
      String[][] a = {{"54","4F","4E","20"},{"77","6E","69","54"},{"6F","65","6E","77"},{"20","20","65","6F"}};
      ...
      for(int j=0;j<t;j++) {
    	  for(int i=0;i<t;i++) {
    		  for(int y=0;y<4;y++) {
    			  for(int x=0;x<4;x++) {
    				  pixel = img.getRGB(x+4*i,y+4*j); 
    				  Color color = new Color(pixel, true);
    				  red = color.getRed();
    		          green = color.getGreen();
    		          blue = color.getBlue();
    		          
    		          st1=Integer.toHexString(red);
    		          st2=Integer.toHexString(green);
    		          st3=Integer.toHexString(blue);
    		          a[x][y]=checkHex(st1);
    		          b[x][y]=checkHex(st2);
    		          c[x][y]=checkHex(st3);
    		  ...
    		  for(int y=0;y<4;y++) {
    			  for(int x=0;x<4;x++) {
    				  Color color=new Color(Integer.parseInt(a[x][y],16),Integer.parseInt(b[x][y],16),Integer.parseInt(c[x][y],16));
    				  img.setRGB(x+4*i, y+4*j, color.getRGB());
    			  }
    		  }
      ...
      file = new File("G:/assignment/sem 5/ISAA/PROJECT/New folder/sample.jpg");
